from flask import Flask, render_template, abort, send_from_directory, request, jsonify, session
import os
...
with open(os.path.join(BASE_DIR, "dialog_scenarios.json"), "r", encoding="utf-8") as f:
    dialog_data = json.load(f)
LEGAL_TOPICS = dialog_data["LEGAL_TOPICS"]
DIALOG_SCENARIOS = dialog_data["DIALOG_SCENARIOS"]
FALLBACK = dialog_data.get("FALLBACK", "Извините, я не смог распознать ваш вопрос. Могу предложить записаться на консультацию или оставить ваш телефон/email для связи с юристом.")
...
def chatbot():
    ...
    if dialog_state.get("step") == "choose_role":
        ...
        elif "медицинское учреждение" in user_message or "учреждение" in user_message:
            session["dialog_state"] = {"role": "медицинское учреждение", "step": "choose_topic"}
    ...
    if dialog_state.get("step") == "choose_topic":
        ...
        topics = LEGAL_TOPICS.get(f"{role}ам", {}) if role != "медицинское учреждение" else LEGAL_TOPICS.get("медицинским учреждениям", {})
        for subtopic, answer in topics.items():
            if subtopic in user_message:
                session["dialog_state"] = {"last_topic": subtopic, "step": 0, "role": role}
                scenario = DIALOG_SCENARIOS.get(subtopic)
                if scenario:
                    resp = scenario[0]
                    return jsonify({"response": answer + " " + resp.get("q"), "options": resp.get("options")})
                else:
    ...
    if "last_topic" in dialog_state:
        ...
        if scenario:
            ...
            if options and any(opt.lower() in user_message for opt in [o.lower() for o in options]):
                if "спасибо, не нужно" in user_message:
                    session["dialog_state"] = {}
                    return jsonify({
                        "response": "Спасибо за обращение! Если появятся вопросы — всегда рад помочь. Всего доброго!",
                        "close_chat": True
                    })
                step = min(step + 1, len(scenario) - 1)
                session["dialog_state"] = {**dialog_state, "step": step}
                resp = scenario[step]
                return jsonify({"response": resp.get("a", resp.get("q")), "options": resp.get("options")})
            if any(word in user_message for word in ["да", "хочу", "интересует", "подробнее", "конечно"]):
                step = min(step + 1, len(scenario) - 1)
                session["dialog_state"] = {**dialog_state, "step": step}
                resp = scenario[step]
                return jsonify({"response": resp.get("a", resp.get("q")), "options": resp.get("options")})
            ...
            if "@" in user_message or "тел" in user_message or "номер" in user_message or user_message.replace(" ", "").isdigit():
                session["dialog_state"] = {}
                return jsonify({"response": "Если вы хотите записаться на консультацию, пожалуйста, позвоните по телефону +7 (917) 834-50-17 или напишите в WhatsApp/Telegram. Мы не собираем и не храним ваши контакты."})
            resp = scenario[step]
            session["dialog_state"] = {**dialog_state, "step": step}
            return jsonify({"response": resp.get("q"), "options": resp.get("options")})
